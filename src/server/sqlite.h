...
static void sqlite_bool_function(sqlite3_context *context, int argc, sqlite3_value **argv) {
    if (argc == 1) {
        ...
        if ((value_type == SQLITE_INTEGER)) {
            ...
            if (!str || *str == 0)
                sqlite3_result_text(context, "false", -1, SQLITE_TRANSIENT);
            else if (!sqlite3_stricmp(str, "false"))
                sqlite3_result_int(context, 0);
            else if (!sqlite3_stricmp(str, "'false'"))
                sqlite3_result_int(context, 0);
            else if (!sqlite3_stricmp(str, "\"false\""))
                sqlite3_result_int(context, 0);
            else if (!sqlite3_stricmp(str, "true"))
...
int sqlite_register_bool_function(sqlite3 *db) {
    return sqlite3_create_function(db,                   // SQLite database connection
